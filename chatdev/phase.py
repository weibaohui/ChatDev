...
class Phase(ABC):
    ...
    def __init__(self,
        ...
        self.max_retries = 3
        self.reflection_prompt = """这是两个角色之间的对话：{conversations} {question}"""
    ...
    def chatting(
        ...
        if not chat_env.exist_employee(assistant_role_name):
            raise ValueError("{assistant_role_name} 在 ChatEnv 中未被招聘。")
        if not chat_env.exist_employee(user_role_name):
            raise ValueError(f"{user_role_name} 在 ChatEnv 中未被招聘。")
        ...
        if need_reflect:
            ...
            if "recruiting" in phase_name:
                if "是".lower() not in seminar_conclusion.lower() and "否".lower() not in seminar_conclusion.lower():
    ...
    def self_reflection(self,
        ...
        if "recruiting" in phase_name:
            question = """回答他们在讨论中的最终结论（是或否），不加其他任何词语，例如，"是" """
        elif phase_name == "DemandAnalysis":
            question = """回答他们在讨论中的最终产品形态，不加其他任何词语，例如，"PowerPoint" """
        elif phase_name == "LanguageChoose":
            question = """总结讨论中用于软件开发的编程语言，格式为："*"，其中 "*" 代表一种编程语言。 """
        elif phase_name == "EnvironmentDoc":
            question = """根据上述列出的代码和文件格式，编写一个 requirements.txt 文件，以指定项目正常运行所需的依赖项或软件包。 """
        else:
            raise ValueError(f"Reflection of phase {phase_name}: Not Assigned.")

        # Reflections actually is a special phase between CEO and 顾问
        ...
        if "recruiting" in phase_name:
            if "是".lower() in reflected_content.lower():
                return "是"
            return "否"
        else:
...
class CodeReviewHuman(Phase):
    ...
    def execute(self, chat_env, chat_turn_limit, need_reflect) -> ChatEnv:
        ...
        log_visualize(
            f"**[人机交互]**\n\n"
            f"现在您可以参与软件的开发！\n"
            f"您的任务是:  {chat_env.env_dict['task_prompt']}\n"
            f"请提供您的反馈（多行输入）。可以是错误报告或新功能需求。\n"
            f"当前反馈索引 #{self.phase_env['cycle_index']} / {self.phase_env['cycle_num']}\n"
            f"在单独的一行上输入 'end' 以提交。\n"
            f"您可以随时输入 \"Exit\" 以退出此模式。\n"
        )
        provided_comments = []
        while True:
            user_input = input(">>>>>>")
        ...
        log_visualize(
            f"**[用户提供的评论]**\n\n 评论 #{self.phase_env['cycle_index']}/{self.phase_env['cycle_num']}: \n\n" +
            self.phase_env["comments"])
...
class TestErrorSummary(Phase):
    ...
    def execute(self, chat_env, chat_turn_limit, need_reflect) -> ChatEnv:
        ...
        if "ModuleNotFoundError" in self.phase_env['test_reports']:
            ...
            log_visualize(
                f"软件测试工程师 发现了 ModuleNotFoundError:\n{self.phase_env['test_reports']}\n")
            pip_install_content = ""
