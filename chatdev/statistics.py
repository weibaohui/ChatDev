    ...
    if os.path.exists(dir):
        ...
        if len(sublines) > 0:
            ...
            if model_type == "GPT_3_5_TURBO" or model_type == "GPT_3_5_TURBO_NEW":
                ...
                model_type = "alibaba/Qwen2-7B-Instruct"
        ...
        lines = open(log_filepath, "r", encoding="utf8").read().split("\n")
        sublines = [line for line in lines if line.startswith("提示词消耗token:")]
        if len(sublines) > 0:
        ...
        lines = open(log_filepath, "r", encoding="utf8").read().split("\n")
        sublines = [line for line in lines if line.startswith("对话消耗token:")]
        if len(sublines) > 0:
            nums = [int(line.split(": ")[-1]) for line in sublines]
        ...
        lines = open(log_filepath, "r", encoding="utf8").read().split("\n")
        sublines = [line for line in lines if line.startswith("合计消耗token:")]
        if len(sublines) > 0:
            nums = [int(line.split(": ")[-1]) for line in sublines]
